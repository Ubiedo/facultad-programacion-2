...
void eliminarJugadorConNombreTJugadoresLDE(TJugadoresLDE &jugador, const char nombre[100]){
  ...
  // conectar el anterior al siguiente, reasignar punteros si estos no son null
  if (buscar->previo != NULL)
  {
    buscar->previo->proximo = buscar->proximo;
  } else // entonces era el primero
  {
    jugador->primero = buscar->proximo;
  }
  if (buscar->proximo != NULL)
  {
    buscar->proximo->previo = buscar->previo;
  } else // entonces era el ultimo
  {
    jugador->ultimo = buscar->previo;
  }
  // elminar el que buscabamos y bajamos la cantidad de jugadores
  jugador->cantidad--;
  liberarTNodo(buscar);
}
